...
const people_type= "Supplier";
  useEffect(() => {
    const fetchSupplier = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_API_BASE_URL}/warehouse/accessory?people_type=${people_type}`, {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
          }
  ...
  const handleAddUser = () => {
    navigate('/add_wareHouse_Accessory');
  };
      ...
      <div className="col-md-4">
        ...
        <div className="col-md-8 text-end">
          <Button variant="primary" onClick={handleAddUser}>
            <MdPersonAdd className="me-2" /> Add warehouse Accessories
          </Button>
        </div>
      </div>
      <div className="row">
