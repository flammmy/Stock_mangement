  ...
  const ShowProduct = () => {
    ...
    const exportToPDF = () => {
      ...
      doc.autoTable({
        head: [['Sr No', 'Lot No', 'Invoice No','Date', 'Shade No', 'Pur. Shade No', 'Length', 'Width', 'Unit', 'Quantity', 'Area (m²)', 'Area (sq. ft.)']],
        body: filteredProducts.map((row) => [row.index, row.lot_no, row.stock_invoice?.invoice_no || 'N/A',row.stock_invoice?.date || 'N/A', row.stock_product?.shadeNo || 'N/A', row.stock_product?.purchase_shade_no || 'N/A', row.length, row.width, row.unit, row.qty, row.area, row.area_sq_ft]),
      });
